...
async def post_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Generate AI content and show preview for manual posting"""
    telegram_id = update.effective_user.id
    ...
    await update.message.reply_text("🤖 Generating AI content for you...\n\nThis may take a moment.")

    try:
        # Get user profile for AI generation
        user_profile = db.get_user_profile(telegram_id)
        if not user_profile:
            await update.message.reply_text("❌ No profile found. Complete onboarding with /start first.")
            return
        ...
        # Generate content using AI (simulated for now - you'd integrate with your AI service)
        ...
        # Generate enhanced story-telling post with bullet points
        import random

        # Story hooks - more engaging openings
        hooks = [
            ...
            f"After years working in {industry}, I've learned that the biggest breakthroughs often come from unexpected places. 🌟",
            f"Last week, I was reminded why {themes} matters more than ever in today's {industry} landscape. 🚀",
            f"Something interesting happened while working on {themes} that I think you'll find valuable. ✨"
        ]

        # Key insights with emojis
        insights = [
            f"💎 {themes.split(',')[0].strip().title()} isn't just about what you know — it's about how you apply it in real-world scenarios.",
            f"🎯 Success in {industry} comes from combining {skills.split(',')[0].strip()} with genuine curiosity and continuous learning.",
            f"🔥 The most valuable skill isn't always technical — it's the ability to adapt and evolve with the industry.",
        ...
        # Action points with emojis
        actions = [
            f"📌 Focus on building genuine connections, not just collecting contacts",
            f"📌 Share your knowledge freely — what you give comes back multiplied",
            f"📌 Ask better questions instead of just seeking quick answers",
            f"📌 Embrace failure as feedback, not as a setback",
            f"📌 Stay curious and never stop learning from those around you"
        ]

        # Engagement questions
        questions = [
            "What's been your biggest learning moment this year? 👇",
            "How do you approach continuous learning in your field? I'd love to hear your thoughts! 💬",
            "What strategies have worked for you? Drop your insights below! 🗣️",
            "Have you experienced something similar? Let's discuss in the comments! 💭",
            "What's your take on this? Share your perspective! 🤝"
        ]

        # Build the post
        selected_hook = random.choice(hooks)
        selected_insights = random.sample(insights, 3)  # 3 insights
        selected_actions = random.sample(actions, 3)    # 3 action points
        selected_question = random.choice(questions)

        # Create hashtags from skills and industry
        skill_tags = [f"#{skill.strip().replace(' ', '')}" for skill in skills.split(',')[:2]]
        ...
        # Generate unique post ID for tracking
        ...
        # Show preview with mobile WebApp option
        keyboard = [
            ...
            [InlineKeyboardButton("🔄 Generate New", callback_data='post_regenerate')],
            [InlineKeyboardButton("❌ Discard", callback_data='post_discard')],
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)

        # Store the generated post in context for later use
